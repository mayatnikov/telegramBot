...
public class Client {
	...
	public Task processMrz(int imageLength, URL tgUrl ) throws Exception {

		// do not use
		// authenticated
		// connection
		URLConnection connection = tgUrl.openConnection();
		InputStream inTelegram = connection.getInputStream();


		URL url = new URL(serverUrl + "/processMrz" );
		return postStreamToUrl(imageLength, inTelegram, url);
	}
...
// ----
private Task postStreamToUrl(int imageSize, InputStream in, URL url) throws Exception {

	byte[] imageContents = new byte[imageSize];
	DataInputStream dis = new DataInputStream(in);
	dis.readFully(imageContents);

	HttpURLConnection connection = openPostConnection(url);
	connection.setRequestProperty("Content-Length", Integer.toString(imageContents.length));
	OutputStream stream = connection.getOutputStream();
	try {
		stream.write(imageContents);
	} finally {
		stream.close();
	}
	return getResponse(connection);
